...
- 2026-07-15 후속 결정. `requesterUserId`를 프롬프트 조립 메서드에 전달하고 `senderId`와 비교해 `[요청자/나]`, `[상대/닉네임]`으로 표시한다. 별도 사용자 조회는 추가하지 않는다.
- Gradle 명령은 실행하지 않았다. 프론트 테스트는 `react-router-dom` 로컬 의존성 누락으로 테스트 로딩 단계에서 중단됐다.
## 2026-07-15 AI Message Tone Refinement

- 현재 입력문만 AI에 전달하며 채팅 기록은 전달하지 않는다.
- 지원 말투는 `SOFT`, `CONCISE`, `POLITE` 세 가지로 제한한다.
- 결과는 자동 전송하지 않고 사용자가 선택한 현재 입력문만 교체한다.
- 방 데이터가 필요 없으므로 draft 방에서도 사용할 수 있다.
- 기존 AI Assist HTTP와 GeminiClient를 재사용하고 WebSocket, Kafka, DB는 추가하지 않는다.
## 2026-07-15 Room Realtime And Personalized AI Decisions

- 영구강퇴는 backend·proto·DB 계약을 유지하고 ChatBox의 버튼과 호출만 제거한다.
- 추방 대상은 현재 방을 보고 있지 않아도 반영되도록 `ROOM_KICKED` 개인 푸시를 받고, FE 전역 상태에서 해당 방 창과 목록을 제거한다.
- 초대 실시간 반영은 클라이언트별 방 재조회 대신 기존 `RoomMemberProfile`을 `RoomFeedResponse`에 추가해 초대된 멤버 정보만 broadcast한다.
- AI 처리 중에는 추천·말투 다듬기·맞춤 추천을 하나의 busy 조건으로 묶어 Enter와 전송 버튼을 모두 막는다.
- 말투 다듬기 결과는 자동 전송하지 않고 입력문 교체 후 4초 결과 모달로 기존 문장과 변경 문장을 보여준다.
- 검색은 현재 로드된 메시지만 대상으로 최신 메시지부터 이동하고 검색어 부분만 `mark`로 강조한다.
- 섬세한 맞춤 메시지는 현재 구현 가능한 텍스트 대화만 사용한다. 프로필 배경·상태 메시지·이미지 분석은 데이터 계약과 VLM 경로가 없어 이번 범위에서 제외한다.
- 맞춤 추천 backend는 요청자가 현재 방의 활성 멤버인지, 선택 상대도 현재 방의 활성 멤버인지 다시 검증한다.
- 상대와의 공통 대화는 요청자가 현재 접근 가능한 방에서 두 사람이 작성한 텍스트만 제한 개수로 조회한다. 현재 방 최근 대화는 별도로 포함해 단체방 맥락을 보존한다.
- 무제한 전체 대화 조회는 DB·Gemini 토큰 비용이 계속 커지므로 공유 대화 최신 180건과 현재 방 최신 30건으로 제한했다.
- 추방 대상은 방 registry에서 먼저 제거한 뒤 남은 멤버에게 broadcast하고, 대상에게는 개인 이벤트를 한 번만 보내 중복 처리를 막는다.
- 초대 이벤트에는 초대 대상의 `RoomMemberProfile`만 추가하고 방 전체 멤버 목록은 재조회하지 않는다.
- JSX 4개 파일은 Babel parser를 통과했고 변경한 MyBatis XML 2개는 XML 파싱을 통과했다. Gradle은 실행하지 않았다.
